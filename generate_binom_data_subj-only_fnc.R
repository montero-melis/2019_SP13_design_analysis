...
# replication. This is a simplified version used for the power analysis which
# only simulates aggregated by-subject data (not trial-level data) from the
# model fitted to the original data set from SP13.

# NB: I've commented out all the code that has to do with items, so that the
# connection with the full function that generates trial-level data is clear.
...
simulate_binom <- function (
  ...
  # 2a) Basic structure
  n_obs <- Nsubj * 4  # total number of observations (or #rows)
  ...
  myprint(fixef)
  # 3b) Plug the intended coefficient for the interaction back in?
