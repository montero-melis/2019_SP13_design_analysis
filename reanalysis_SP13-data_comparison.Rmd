...
d$mv <- scale(as.numeric(d$movement), scale = FALSE) * -2
d$wt <- scale(as.numeric(d$word_type), scale = FALSE) * -2
d$mv_wt <- d$mv * d$wt
```
...
Fit pairs of models, one is the actual full model of interest, the other is
identical, except that it does not contain the interaction between Movement and Word type:
...
fit_brm <- function(formu_str, RE, df = d) {
  ...
  l <- list(full, null, BF)
  saveRDS(l, paste("sims_etc/sp13_bfm_", RE, ".rds", sep = ""))
  l
}
```
...
#   RE = "max"
#   )
# # No RE correlation models
# bfm_nocorr <- fit_brm(
#   formu_str = "errors | trials(n) ~ 1 + mv + wt + mv_wt + (1 + mv + wt + mv_wt || subject)",
#   RE = "nocorr"
#   )
# # RE intercept only models
# bfm_interc <- fit_brm(
#   formu_str = "errors | trials(n) ~ 1 + mv + wt + mv_wt + (1 | subject)",
#   RE = "interc"
#   )
```
...
```{r}
bfm_max    <- readRDS("sims_etc/sp13_bfm_max.rds")
bfm_nocorr <- readRDS("sims_etc/sp13_bfm_nocorr.rds")
bfm_interc <- readRDS("sims_etc/sp13_bfm_interc.rds")
```

```{r}
show_models <- function(bfm, BF_only = FALSE) {
  if (! BF_only) map(bfm[1:2], function(x) print(summary(x)))
  print("BAYES FACTOR FROM BRIDGE SAMPLING:")
  print(bfm[[3]])
}
```
...
```{r}
show_models(bfm_max)
show_models(bfm_nocorr)
show_models(bfm_interc)
```
...
`round(fixef(bfm_binom)[1, 1], 2)` log-odds is small. For instance, 
