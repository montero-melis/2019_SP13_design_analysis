...
plot_sims <- function(type, df = sims_BIC) {
  d <- sims_BIC %>%
    filter(sim_type == type, Nsubj %in% c(15, 60, 102), converged == TRUE) %>%
  ...
  if (type == "type2") {
    ...
    mycolours <- c("#FF3300", "#FFFF33", "#33CC33")
    mytitle <- "Type 2 simulations / power (effect present)"
    yinterc <- 0.9
    } else if (type == "type1") {
      mylevels <- c("Accept H1", "Inconclusive", "Accept H0")
      mycolours <- c("#FF3300", "#FFFF33", "#33CC33")
      mytitle <- "Type 1 simulations (effect absent)"
      yinterc <- 0.95
      } else {
...
```{r}
sims_BIC %>%
  filter(Nsubj %in% c(15, 60, 102), converged == TRUE) %>%
  select(rseed : Nsubj, BF_BIC) %>%
  unique() %>%
  mutate(BF_decision = ifelse(
    BF_BIC >= 6, "Accept H1", ifelse(
