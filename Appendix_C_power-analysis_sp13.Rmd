...
converge, which is specially frequent when sample sizes are small).


```{r}
# First a function that computes how many simulations we still need to run.
# It outputs FALSE if it can't find a file with simulations. This function is
# called from the run_till_aim() function which actually runs the simulations.
...
# run_till_aim(1000, filter_out = c(15))
```
