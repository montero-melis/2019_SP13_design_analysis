...
Fit model with same priors we will use in the replication
----------------------------
...
Specify weakly informative priors $N(0,\sigma^2 = 4)$ for population-level fixed effects:

```{r}
myprior <- set_prior("normal(0, 2)", class = "b")  
# NB: In Stan normal is specified with sigma, see
...
#   save_all_pars = TRUE
#   )
# saveRDS(bfm_binom_normprior, "sims_etc/bayes_glmm_normprior_interact.rds")
# # without interaction
# bfm_binom_normprior_nointeract <- update(
#   bfm_binom_normprior, formula = ~ . -movement : word_type  # Same but without the interaction term
#   )  
# saveRDS(bfm_binom_normprior_nointeract, "sims_etc/bayes_glmm_normprior_nointeract.rds")
```
