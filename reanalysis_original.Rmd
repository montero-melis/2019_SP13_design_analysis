...
*NB*:
The data were kindly shared with us by the original authors, but we have agreed
not to further distribute it and for this reason this script cannot be reproduced.
The data is only on our disk and not included in this git repository.
...
```{r, message=FALSE}
# Load data
d_allconds <- read_csv("orig_study/SP13_orig-data_total-errors_long-format.csv")
...
critical arm/leg movement conditions, so we subset the relevant data:
...
ggsave("fig_original_results.png",
...
Yes, pretty much: The F-values are identical, but I don't know how they computed
MSE. Their numbers don't line up with the current results.



## Anova on critical 2x2 design
...
interaction (p.226). However, it is not clear how they arrived at this figure.
...
```{r}
d$eff_word <- ifelse(substr(d$movement,1,3) == d$word_type, "same", "diff")
head(d, 4)
d_interact <- d %>% 
...
head(d_4ttest, 3)
with(d_4ttest, t.test(diff, same, paired = TRUE))
```
...
with(d_4ttest, (mean(same) - mean(diff)) / sd(same - diff))
# # Note we get the same value using the lsr package
# with(d_4ttest, lsr::cohensD(diff, same, method = "paired"))
```

```{r, include=FALSE}
d_09 <- round(with(d_4ttest, (mean(same) - mean(diff)) / sd(same - diff)), 2)
```
...
is plugged in. (If we use the `eta.sq.part` value instead, it seems completely
off with $d$ close to 2.) Either way it doesn't match the value reported in SP13.
...
We fit a binomial model using the default options for priors.
...
Is the effect really supported by the data?
-------------------------------

*brms* allows us to perform "non-linear hypothesis testing". This lets us test
the hypothesis that the crucial interaction is larger than zero (i.e., there is
...
The evidence ratio that there exists an interference effect is overwhelming.
...
Save posterior distribution of interaction coefficient to use as effect size in BFDA
-----------------------------------------
...
We first fit the equivalent model to the above:

```{r}
fm_binom <- glmer(cbind(errors, n - errors) ~ 1 + movement * word_type + (1 | subject),
...
comparable. One estimate that is higher in the `brms` model is the subject-level
SD. However, we can also see that the `lme4` estimate is well contained in the
95% C.I. of the `brms` model.

In sum, both ways of analyzing the data (`brms` and `lme4`) yield highly
similar results.
...
to a conclusion that would not scale up to the larger population (or it might
not even replicate within that participant sample if the experiment was run
anew with the same participants).
