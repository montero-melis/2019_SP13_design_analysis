...
But is the effect really supported by the data?
-------------------------------
...
Save posterior distribution of interaction coefficient to use as effect size prior for BFDA
============================================
...
- The estimated average odds of making an error (intercept) is 
$e ^ {`r round(fixef(fm_binom)[1], 2)`} = `r round(exp(fixef(fm_binom)[1]), 2)`$
(corresponding to a probability of
`r round(inv.logit(fixef(fm_binom)[1]), 2)`)
- Neither movement condition (`movement`) or the type of word (`word_type`)
have a significant effect on the odds of making an error per se (i.e., no main
effects).
- The critical interaction, however, is significant and tells us that the odds of making
...
In other words, participants were `r round(exp(fixef(fm_binom)[4]), 2)` times
more likely to make an error if effector and semantics match than if they mismatch.
This is the *interference effect* of interest!
...
identical
(*brms*: $`r round(fixef(bfm_binom)[4,1], 3)`$;
*lme4*: $`r round(fixef(fm_binom)[4], 3)`$).
