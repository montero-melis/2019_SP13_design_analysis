...
## Computing effect size for critical interaction in the ANOVA
...
Eta squared is the most commonly used measure of effect size for ANOVAs: It 
...
```{r}
DescTools::EtaSq(aov_orig, type = 1)
# sjstats::eta_sq(aov_orig)  # yields same results
```


### Where does the originally reported Cohen's $d$ come from?

As mentioned above, SP13 report an effect size for the critical interaction of
Cohen's $d = 1.25$ (SP13, p. 226).
Even though the results from the ANOVA above mostly line up with SP13's results,
I don't know how this figure for $d$ was obtained. 

What I've tried:

- [This online calculator](https://www.psychometrica.de/effect_size.html)
converts between different effect size measures (see section 13). When plugging
in $\eta^2 = 0.04$, the corresponding effect size is $d = 0.41$, which is very
far off from the figure in SP13!
- [This IBM support website](https://www-01.ibm.com/support/docview.wss?uid=swg21476421)
cites formulae to convert between *partial* Eta-squared, Cohen's $f$, and
Cohen's $d$: 
\[
f = \sqrt{ \frac{\eta^2}{1 - \eta^2} };
d = 2*f
\]
This formular yields the same value as above ($d=0.41$) if the `eta.sq` value
is plugged in. If we use the `eta.sq.part` value instead, it seems completely
off ($d$ close to 2).


### Could it be that the *d* value comes from a t-test?
...
In conclusion: No, we can't reproduce their Cohen's $d$ in this way either.
