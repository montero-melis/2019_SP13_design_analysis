...
# Simulations needed to get to an aim
sim_till_aim(1000)
...
```{r}
sim_till_aim(100)
# run_till_aim(100, filter_out = c(15))
```
...
```{r}
with(sims_BIC, addmargins(table(Nsubj, converged)))
```
...
```{r}
by(sims_BIC$BF_BIC, with(sims_BIC, list(Nsubj, converged)), summary)
```
