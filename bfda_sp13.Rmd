...
Simulate hypothetical studies under *minimum effect size* and default prior
-----------------------------
...
```{r}
SSD(sim.H1_05_defprior, power=.80, boundary=c(1/6, 6), alpha = 0.05)
```
...
Sanity check 1: BFDA using a *minimum effect size of interest* + prior.mean = ES
============================
...
Sanity check 2: BFDA using a *minimum effect size of interest* + prior.mean = ES + Var = 1
============================

As a 2nd sanity check, run a simulation like the previous one but with a more
reasonable variance on the prior (i.e., 1).
...
```{r}
# # Under alternative hypothesis H1
# sim.H1_05_prior05_var1 <- BFDA.sim(expected.ES = 0.5,  # minimum ES of interest
#                       type = "abtest",
...
#                       B=2000, verbose=TRUE, cores=1)
# 
# # [1] "Simulation started at 2019-08-15 14:23:24"
# # [1] "Simulation finished at 2019-08-15 14:34:59"
# # Duration: Time difference of 11.58677 mins
# 
# # Save to disk:
# write_rds(sim.H1_05_prior05_var1, "sims_etc/sim_H1_05_prior05_var1.rds")
```


```{r}
# Load from disk
sim.H1_05_prior05_var1 <- read_rds("sims_etc/sim_H1_05_prior05_var1.rds")
sim.H1_05_prior05_var1
```
...
```{r}
# # Under null hypothesis H0
# sim.H0_05_prior05_var1 <- BFDA.sim(expected.ES = 0,  # no effect
#                       type = "abtest",
#                       prior=list("normal", list(prior.mean = 0.5, prior.variance = 1)),
#                       options.sample = list(effecttype = "logOR"),
#                       n.min=60, n.max=300, stepsize = 20, boundary=Inf,
...
#                       B=2000, verbose=TRUE, cores=1)
# 
# # [1] "Simulation started at 2019-08-15 14:35:45"
# # [1] "Simulation finished at 2019-08-15 14:47:29"
# # Duration: Time difference of 11.73509 mins
# 
# # Save to disk:
# write_rds(sim.H0_05_prior05_var1, "sims_etc/sim_H0_05_prior05_var1.rds")
```

```{r}
# Load from disk:
sim.H0_05_prior05_var1 <- read_rds("sims_etc/sim_H0_05_prior05_var1.rds")
sim.H0_05_prior05_var1
```
...
```{r}
BFDA.analyze(sim.H1_05_prior05_var1, design="sequential", n.min=60, n.max=300, boundary=6)
```

```{r}
plot(sim.H1_05_prior05_var1, n.min=60, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H0_05_prior05_var1, design="sequential", n.min=60, n.max=300, boundary=6)
```

```{r}
plot(sim.H0_05_prior05_var1, n.min=60, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H1_05_prior05_var1, design="sequential", n.min=60, n.max=140, boundary=6)
```

```{r}
plot(sim.H1_05_prior05_var1, n.min=60, n.max=140, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H0_05_prior05_var1, design="sequential", n.min=60, n.max=140, boundary=6)
```

```{r}
plot(sim.H0_05_prior05_var1, n.min=60, n.max=140, boundary=c(1/6, 6))
```
...
```{r}
SSD(sim.H1_05_prior05_var1, power=.80, boundary=c(1/6, 6))
```
...
Simulations for letter to Cortex editors
============================

We run simulations with much higher N to get at what N we would actually need
to plan for compelling evidence.

We run two simulations, both with `expected.ES` equal to the posterior from our
re-analysis of the SP13 data. 

Only the `prior` argument varies, with the following two options:

1. A non-informative default prior (to convince a "skeptical audience", see SW18).
2. A prior that is equal to the posterior distribution based on original results.


Simulate hypothetical studies
-----------------------------

For each of the settings for prior, simulate data under H1 and H0 and save to
disk:
...
# 
# 
# ## Informed prior (based on results in SP13):
# 
# # Under alternative hypothesis H1
# sim.H1_origES_origPrior <- BFDA.sim(
#   expected.ES = beta_post,  # posterior of original becomes prior
#   type = "abtest",
#   prior=list("normal", list(prior.mean = mean(beta_post), prior.variance = var(beta_post))),
#   options.sample = list(effecttype = "logOR"),
#   n.min=100, n.max=3000, stepsize = 100, boundary=Inf,
#   alternative="greater",
#   B=1000, verbose=TRUE, cores=1)
# 
# # [1] "Simulation started at 2019-08-16 10:21:19"
# # [1] "Simulation finished at 2019-08-16 10:29:42"
# # Duration: Time difference of 8.379531 mins
# 
# # Save to disk:
# write_rds(sim.H1_origES_origPrior, "sims_etc/sim_H1_origES_origPrior.rds")
# 
# # Under null hypothesis H0
# sim.H0_origES_origPrior <- BFDA.sim(
#   expected.ES = 0,  # no effect
#   type = "abtest",
#   prior=list("normal", list(prior.mean = mean(beta_post), prior.variance = var(beta_post))),
#   options.sample = list(effecttype = "logOR"),
...
#   B=1000, verbose=TRUE, cores=1)
# 
# # [1] "Simulation started at 2019-08-16 10:29:43"
# # [1] "Simulation finished at 2019-08-16 10:38:06"
# # Duration: Time difference of 8.385818 mins
# 
# # Save to disk:
# write_rds(sim.H0_origES_origPrior, "sims_etc/sim_H0_origES_origPrior.rds")
```
...
sim.H1_origES_origPrior <- read_rds("sims_etc/sim_H1_origES_origPrior.rds")
sim.H1_origES_origPrior

sim.H0_origES_origPrior <- read_rds("sims_etc/sim_H0_origES_origPrior.rds")
sim.H0_origES_origPrior
```
...
```{r}
BFDA.analyze(sim.H1_origES_defPrior, design="sequential", n.min=100, n.max=3000, boundary=6)
```

Informed prior:

```{r}
BFDA.analyze(sim.H1_origES_origPrior, design="sequential", n.min=100, n.max=3000, boundary=6)
```
...
```{r}
plot(sim.H1_origES_defPrior, n.min=100, boundary=c(1/6, 6))
```

Informed:

```{r}
plot(sim.H1_origES_origPrior, n.min=100, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H0_origES_defPrior, design="sequential", n.min=100, n.max=3000, boundary=6)
```

Informed

```{r}
BFDA.analyze(sim.H0_origES_origPrior, design="sequential", n.min=100, n.max=3000, boundary=6)
```

Default:

```{r}
plot(sim.H0_origES_defPrior, n.min=100, boundary=c(1/6, 6))
```

Informed:

```{r}
plot(sim.H0_origES_origPrior, n.min=100, boundary=c(1/6, 6))
```
...
```{r}
SSD(sim.H1_origES_defPrior, power=.80, boundary=c(1/6, 6))
```

With informed prior:

```{r}
SSD(sim.H1_origES_origPrior, power=.80, boundary=c(1/6, 6))
```
...
```{r}
SSD(sim.H0_origES_defPrior, power=.80, boundary=c(1/6, 6))
```

With informed prior:

```{r}
SSD(sim.H0_origES_origPrior, power=.80, boundary=c(1/6, 6))
```
...
Proposed design in draft letter to Cortex
============================

1.	Set minN=64 participants, which is over four times the sample size of the original. 
2.	At N=64, compute the BF with a weakly informative prior. 
3.	If BF10 > 6 or BF01 > 6, stop data collection and report results (see below). Else:
4.	While N < 96, collect another batch of 16 participants and repeat step 3.
5.	If we reach N=96, stop data collection, compute BFs and report results.
...
```{r}
# # Under alternative hypothesis H1 with ES = 0.5
# sim.H1_ES05_defPrior_steps16 <- BFDA.sim(
#   expected.ES = 0.5,
#   type = "abtest",
#   prior=list("normal", list(prior.mean = 0, prior.variance = 1)),
#   options.sample = list(effecttype = "logOR"),
...
#   B=2000, verbose=TRUE, cores=1)
# 
# # [1] "Simulation started at 2019-08-18 11:28:42"
# # [1] "Simulation finished at 2019-08-18 11:32:19"
# # Duration: Time difference of 3.618018 mins
# 
# # Save to disk:
# write_rds(sim.H1_ES05_defPrior_steps16, "sims_etc/sim_H1_ES05_defPrior_steps16.rds")
# 
# 
...
```{r}
BFDA.analyze(sim.H1_ES05_defPrior_steps16, design="sequential", n.min=64, n.max=112, boundary=6)
plot(sim.H1_ES05_defPrior_steps16, n.min=64, boundary=c(1/6, 6))
```

ES = 0.7

```{r}
BFDA.analyze(sim.H1_ES07_defPrior_steps16, design="sequential", n.min=64, n.max=112, boundary=6)
plot(sim.H1_ES07_defPrior_steps16, n.min=64, n.max = 112, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H1_13_defPrior_steps16, design="sequential", n.min=64, n.max=112, boundary=6)
plot(sim.H1_13_defPrior_steps16, n.min=64, boundary=c(1/6, 6))
```
...
```{r}
BFDA.analyze(sim.H0_defPrior_steps16, design="sequential", n.min=64, n.max=112, boundary=6)
plot(sim.H0_defPrior_steps16, n.min=64, boundary=c(1/6, 6))
```
...
```{r}
SSD(sim.H1_ES05_defPrior_steps16, power=.80, boundary=c(1/6, 6))
SSD(sim.H1_ES07_defPrior_steps16, power=.80, boundary=c(1/6, 6))
SSD(sim.H1_13_defPrior_steps16, power=.80, boundary=c(1/6, 6))
```
